...
class OntologyEncoder(object):
  ...
  def encode_ont(self, syn_names=None):
    ...
    for syn in leaves + leaf_non_leaves:
      leaf_syn_index[syn.name()] = len(leaf_syn_index)
    ...
    while len(real_non_leaves) != len(non_leaf_reps):
      ...
      for nl, known_nl_hyps in real_non_leaves:
        if nl.name() in non_leaf_syn_index:
          continue
        nl_hyp_reps = []
        unrepresented = False
        for hyp in known_nl_hyps:
          if hyp.name() in leaf_syn_index:
            nl_hyp_reps.append(leaf_reps[leaf_syn_index[hyp.name()]])
          elif hyp.name() in non_leaf_syn_index:
            nl_hyp_reps.append(non_leaf_reps[non_leaf_syn_index[hyp.name()]])
          else:
        ...
        if unrepresented:
          continue
        non_leaf_syn_index[nl.name()] = len(non_leaf_syn_index)
    ...
    for leaf in leaf_syn_index:
      ind = leaf_syn_index[leaf]
      ont_index[leaf] = ind
      ont_rep.append(leaf_reps[ind])

    num_leaves = len(ont_rep)

    for nonleaf in non_leaf_syn_index:
      ind = non_leaf_syn_index[nonleaf]
      ont_index[nonleaf] = ind + num_leaves
      ont_rep.append(non_leaf_reps[ind])
